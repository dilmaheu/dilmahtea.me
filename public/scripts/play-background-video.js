{
  ...
  backgroundVideo.addEventListener("timeupdate", () => {
    if (backgroundVideo.currentTime > 0.5) {
      backgroundVideo.currentTime = Math.random();
    }
  });

  const interactionEvents = ["click", "touchstart", "keydown"];

  const firstInteractionEventListener = async () => {
    await backgroundVideo.play();

    interactionEvents.forEach((eventType) => {
      document.removeEventListener(eventType, firstInteractionEventListener);
    });
  };
