...
const filteredData = selectedPages.reduce((result, page) => {
  if (page in data.data) {
    ...
    const pageArray = Array.isArray(pageData.data) ? pageData.data : [pageData.data];

    const pageFilteredData = pageArray.map(({ attributes }) => {
      const filteredObj = {
        locale: attributes?.locale,
        Title: attributes?.Title || attributes?.Estate_name,
        Intro_text: attributes?.Intro_text?.slice(0, 60).replace(/(<([^>]+)>)/gi, ""),
        Intro_blob: {
          url: CMS_ENDPOINT + (attributes?.Intro_blob?.data?.attributes?.formats?.thumbnail?.url || attributes?.Intro_blob?.data?.attributes?.url),
      ...
      if (attributes?.localizations?.data?.length) {
        filteredObj.localizations = attributes.localizations.data.map(({ attributes }) => ({
          locale: attributes?.locale,
          Title: attributes?.Title || attributes?.Estate_name,
          Intro_text: attributes?.Intro_text?.slice(0, 60).replace(/(<([^>]+)>)/gi, ""),
          Intro_blob: {
